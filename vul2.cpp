...
BOOL CopyStream(FILE *Src,FILE *Dest)
{
 ...
 // VULN: Casting an unsigned int to int can overflow the int
 FileSize= (int) GetFileSize(Src);
...
int JPGtoPDF(const char *OpenName,const char *SaveName)
{
 ...
 FILE  *JPGStream,*AStream; 
    ...
    // VULN: Removed error checking so invalid file attempts access.
    /*
    if(JPGStream==NULL)
    {
